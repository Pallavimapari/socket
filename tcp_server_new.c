...
int main(int argc, char* argv[])
{
	...
	serv_addr.sin_addr.s_addr = INADDR_ANY; //can be any IP address like "your system IP/loopback/localhost/ethernet"
                                               // e.g. "192.168.1.4"
	//serv_addr.sin_addr.s_addr = htonl("192.168.2.103");  //specific ip must in network byte order
...
//		free(connfd);
		...
		pthread_create(&tid[i++], NULL, thread_fun, &connfd);
		...
		printf("\n connfd socket descriptor is created sucussfully having number as %d\n",connfd);
		}
